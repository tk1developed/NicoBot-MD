...
let about = (await this.fetchStatus(user).catch(console.error) || {}).status || '×'
            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@bio', about).replace('@readMore', global.readMore).replace('@subject', await m.conn.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*⚠️ ESTE GRUPO NO TIENE DESCRIPCIÓN ⚠️*') :
...
this.sendMessage(id, { text: text, contextInfo:{ mentionedJid:[user], "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "title": `${action === 'add' ? '🍬 ᭅ᚜ʙ͜ɪᴇᷱ͢ɴͤᴠᷝ͜ᴇͨɴͦ͢ɪͫᴅͤ͜ᴏ͢͢᚛ 🍓ᭃ' : '🍫 𓆩͜ꥇ ᮫ׄ߭︵݊᷼꯬𝆂ᴀ꯬߭݉᷼ᴅ𝆭i꯬߭݉᷼ᴏs꯬߭݉᷼︵ꥇ᮫ ߭͜𓆪 🍬'}`, "body": packname, "previewType": "PHOTO", "thumbnailUrl": ``, "thumbnail": apii.data, "sourceUrl": yt}}})
}}}
      break;
    case 'promote':
    case 'daradmin':
