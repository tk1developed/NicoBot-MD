...
╚═════════════════════╝
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 30000 && !isROwner) {
            if (user.commandCount === 2) {
              ...
              if (remainingTime > 0) {
                const messageText = `*[ ⏳ ] e̳s̳p̳e̳r̳a̳ ${remainingTime} s̳e̳g̳u̳n̳d̳o̳s̳ ̳a̳n̳t̳e̳s̳ ̳d̳e̳ ̳u̳s̳a̳r̳ ̳o̳t̳r̳o̳ ̳c̳o̳m̳a̳n̳d̳o̳*`;
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: '➢ ⃟⃞❆𖤍𝘋𝘪𝘦𝘨𝘰-𝘚𝘢𝘬𝘶𝘳𝘢𝘉𝘰𝘵𖤍𖣘 ⃟⃞❆', thumbnail: imagen1, sourceUrl: 'https://chat.whatsapp.com/KXjS9CBRF680GBf8fiXDeu'}}}}, aa);
