...
const handler = async (m, {conn, usedPrefix, usedPrefix: _p, __dirname, text, isPrems}) => {
  ...
  const fkon = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: `status@broadcast` } : {}) }, message: { 'contactMessage': { 'displayName': wm, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;${wm},;;;\nFN:${wm},\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabell:Ponsel\nEND:VCARD`, 'jpegThumbnail': imagen1, thumbnail: imagen1 ,sendEphemeral: true}}};
    await conn.reply(m.chat, '🍓 𝐏𝐫𝐨𝐧𝐭𝐨 𝐒𝐞 𝐄𝐧𝐯𝐢𝐚𝐫𝐚 𝐄𝐥 𝐌𝐞𝐧𝐮 𝐃𝐞𝐥 𝐁𝐨𝐭 . . .',m, { contextInfo:{ forwardingScore: 2022, isForwarded: true, externalAdReply: {title: '🦋 𝗦𝖺𝗄𝗎𝗋𝖺 - 𝗕𝗈𝗍 - 𝗟𝗂𝗍𝖾 : 𝗠𝖣', body: '👋🏻 𝚄𝙽  𝙻𝙸𝙽𝙳𝙾 𝙳𝙸𝙰 𝙳𝙸𝙰 𝙿𝙰𝚁𝙰 𝚃𝙸 𝙱𝙴𝙱𝙴', sourceUrl: global.md, thumbnail: await (await fetch(pp)).buffer() }}})
//m.react('🍓');
    await conn.sendMessage(m.chat, { react: { text: '🚀', key: m.key } })
  let txt =`╭✨️⸽⃕𝕾𝖆𝖐𝖚𝖗𝖆-𝕭𝖔𝖙-𝕷𝖎𝖙𝖊-𝕸𝕯🍁⃨፝⃕✰
┋ 🕊️𝐃𝐮𝐞𝐧̃𝐨 𝐃𝐞𝐥 𝐁𝐨𝐭: Diego
┋ 🍓𝐁𝐨𝐭 𝐎𝐟𝐢𝐜𝐢𝐚𝐥: +57 310 6909511
...
handler.help = ['menu'];
handler.tags = ['menu'];
handler.command = /^(menu|allmenu|help|ayuda)$/i;
