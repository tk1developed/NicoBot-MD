...
const handler = async (m, { conn, args }) => {
   const res = await fetch('https://api.github.com/repos/diegojadibot/Multiverse-MD');
   const json = await res.json();
   let txt = `           *乂  B O T S C R I P T  乂*\n\n`;
      txt += `◦  *Nombre:* ${json?.name || 'MultiverseBot-MD'}\n`;
   ...
   await conn.sendMessage(m.chat, {text: txt.trim(), mentions: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), contextInfo: {forwardingScore: 9999999, isForwarded: true, mentionedJid: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "renderLargerThumbnail": true, "title": global.titulowm2, "containsAutoReply": true, "mediaType": 1, "thumbnail": imagen6, "mediaUrl": `https://github.com/diegojadibot/Multiverse-MD`, "sourceUrl": `https://github.com/diegojadibot/Multiverse-MD`}}}, {quoted: m});
};
handler.help = ['scbot'];
handler.tags = ['info'];
