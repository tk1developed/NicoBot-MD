...
const handler = async (m, { conn, text }) => {
 if (!text) throw `🌻 Ingrese el nombre de alguna canción de spotify.`;
  try {
    ...
    let letra;
    letra = `${letra_s ? letra_s + '\n\n🌟 Botsito Yoshiko' : '✨️ YoshikoBot-MD'}`  
    const tags = {
    ...
    await conn.sendMessage(m.chat, {text: spotifyi.trim(), contextInfo: {forwardingScore: 9999999, isForwarded: true, "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "renderLargerThumbnail": true, "title": wm, "containsAutoReply": true, "mediaType": 1, "thumbnail": img, "thumbnailUrl": img, "mediaUrl": linkDL, "sourceUrl": linkDL}}}, {quoted: fkontak});
