 ...
 if (enviando) return;
  ...
  try {
    ...
    if ( isPrems || isMods || isOwner || m.fromMe) {
      ...
      for (const entry of dataArray) await conn.sendMessage(entry + '@s.whatsapp.net', {text: '✉️ *SOLICITUD PARA UN GRUPO*\n\n*—◉ 👤 SOLICITANTE:* ' + '@' + m.sender.split('@')[0] + '\n*—◉ 📝 ENLACE DEL GRUPO:* ' + link, mentions: [m.sender], contextInfo: {forwardingScore: 9999999, isForwarded: true, mentionedJid: [m.sender], "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "renderLargerThumbnail": true, "title": global.titulowm2, "containsAutoReply": true, "mediaType": 1, "thumbnail": imagen6, "mediaUrl": `${link}`, "sourceUrl": `${link}`}}}, {quoted: m});
      enviando = false 
    }
  } catch {
    enviando = false 
