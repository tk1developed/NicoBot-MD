...
const smsMalused2 = () => { return `${lenguajeSK['smsAvisoMG']()}🐈 *DEBE DE USAR EL COMANDO COMO EN ESTE EJEMPLO:*\n`}
const smsMalused3 = () => { return `${lenguajeSK['smsAvisoMG']()}🐈 *RESPONDE A UN MENSAJE CON EL COMANDO O USE ESTE EJEMPLO:*\n`}

//Error
const smsMalError = () => { return `${lenguajeSK['smsAvisoFG']()}\`\`\`OCURRIÓ UN ERROR INESPERADO.\`\`\``}
const smsMalError2 = () => { return `${lenguajeSK['smsAvisoFG']()}\`\`\`SURGIÓ UN INCONVENIENTE.\`\`\`\n`}
const smsMalError3 = () => { return `${lenguajeSK['smsAvisoFG']()}\`\`\`ALGO SALIÓ MAL, REPORTE ESTE COMANDO USANDO:\`\`\`\n`}
...
//convertidores.js
const smsConURL = () => { return lenguajeSK.smsAvisoMG() + '*RESPONDA A UNA IMAGEN PARA CONVERTIR A ENLACE*'} 
...
//convertidores.js
const smsConVIDEO = () => { return lenguajeSK.smsAvisoMG() + '*RESPONDA A STICKER EN MOVIMIENTO PARA CONVERTIR A VÍDEO*'} 
const smsConVIDEO2 = () => { return lenguajeSK.smsAvisoMG() + '*SÓLO STICKER EN MOVIMIENTO PARA CONVERTIR A VÍDEO*'} 
const smsConVIDEO3 = () => { return lenguajeSK.smsAvisoEG() + '*✓ AQUÍ TIENES TÚ VÍDEO!!*'} 

//convertidores.js
const smsConGIF = () => { return lenguajeSK.smsAvisoMG() + '*RESPONDA A UN VÍDEO PARA CONVERTIR A GIF CON AUDIO*'} 
const smsConGIF2 = () => { return lenguajeSK.smsAvisoMG() + '*RESPONDA A UN VÍDEO. EL TIPO DE ARCHIVOS USADO NO ES CORRECTO* '} 
const smsConGIF3 = () => { return lenguajeSK.smsAvisoEG() + '*GIF CON AUDIO (Abra el gif para escuchar el audio)*'} 

//convertidores.js
const smsConVN = () => { return lenguajeSK.smsAvisoMG() + '*RESPONDA A UN VÍDEO O AUDIO PARA CONVERTIR A NOTA DE VOZ*'} 
const smsConVN1 = () => { return lenguajeSK.smsAvisoFG() + '*ALGO NO SALIÓ BIEN CON EL VÍDEO, INTENTE DE NUEVO*'} 
const smsConVN2 = () => { return lenguajeSK.smsAvisoFG() + '*ALGO NO SALIÓ BIEN CON EL AUDIO, INTENTE DE NUEVO*'} 
const smsConVN3 = () => { return lenguajeSK.smsAvisoFG() + '*ALGO NO SALIÓ BIEN AL TRATAR DE CONVERTIR SU AUDIO A NOTA DE VOZ*'}
const smsConVN4 = () => { return lenguajeSK.smsAvisoFG() + '*ALGO NO SALIÓ BIEN AL TRATAR DE CONVERTIR SU VÍDEO A NOTA DE VOZ*'} 

//convertidores.js
const smsTradc1 = () => { return lenguajeSK.smsAvisoMG() + '*RESPONDA A UN MENSAJE DE TEXTO O ESCRIBA EL CÓDIGO Y TEXTO PARA CONVERTIR A AUDIO, EJEMPLO:*\n'}
const smsTradc2 = () => { return ' (codigo) (texto)\n'} 
const smsTradc3 = () => { return lenguajeSK.lenguaje() + ' Este texto será audio\n\n'}
...
//grupos-admins.js
const smsSetW = () => { return `${lenguajeSK['smsAvisoEG']()}*LA BIENVENIDA DEL GRUPO HA SIDO CONFIGURADA*`}
...
//grupos-admins.js
const smsSetB = () => { return `${lenguajeSK['smsAvisoEG']()}*LA DESPEDIDA DEL GRUPO HA SIDO CONFIGURADA*`}
const smsSetB2 = () => { return `${lenguajeSK['smsAvisoIIG']()}🙌 *_ESCRIBA EL MENSAJE DE DESPEDIDA_*\n*_OPCIONAL PUEDE USAR LO QUE ESTA CON "@" PARA AGREGAR MÁS INFORMACIÓN:_*\n\n*⚡ @user (Mención al usuario(a))*\n\n*RECUERDE QUE EL "@" ES OPCIONAL*`}

//grupos-admins.js
const smsDest = () => { return `${lenguajeSK['smsAvisoEG']()}*LA DESCRIPCIÓN DEL GRUPO HA SIDO CONFIGURADA*`}

//grupos-admins.js
const smsNam1 = () => { return `${lenguajeSK['smsAvisoEG']()}*EL NOMBRE DEL GRUPO HA SIDO CONFIGURADO*`}
const smsNam2 = () => { return `${lenguajeSK['smsAvisoMG']()}*🙌 ESCRIBA EL NUEVO NOMBRE DEL GRUPO*`}
const smsNam3 = () => { return `${lenguajeSK['smsAvisoFG']()}*EL NOMBRE DEL GRUPO NO DEBE DE TENER MÁS DE 25 CARACTERES*`}

//grupos-admins.js
const smsGrupoPP = () => { return `${lenguajeSK['smsAvisoMG']()}*RESPONDA A UNA IMAGEN PARA CAMBIAR LA FOTO DEL GRUPO*`}
const smsGrupoPP2 = () => { return `${lenguajeSK['smsAvisoEG']()}*🪄 LA IMAGEN DEL GRUPO FUE ACTUALIZADA!!*`}

//grupos-admins.js
const smsRestGp = () => { return `${lenguajeSK['smsAvisoEG']()}*EL ENLACE DEL GRUPO HA SIDO RESTABLECIDO*`}

//grupos-admins.js
const smskick1 = () => { return `${lenguajeSK['smsAvisoAG']()}*ETIQUETE A LA PERSONA O RESPONDA AL MENSAJE DE LA PERSONA QUE QUIERE ELIMINAR*\n\n*EJEMPLO: `}
...
//grupos-admins.js
const smsGrupoOpen = () => { return `${lenguajeSK['smsAvisoEG']()}*YA PUEDEN ESCRIBIR TODOS!!*`}
const smsGrupoClose = () => { return `${lenguajeSK['smsAvisoEG']()}*GRUPO CERRADO, SÓLO ADMINS PUEDEN ESCRIBIR!!*`}
...
const smsPropban2 = (bot) => { return `${lenguajeSK['smsAvisoFG']()}*@${bot} NO PUEDE SER BANEADO CON ESTE COMANDO* 😹`}
const smsPropban3 = (ownerNumber) => { return `${lenguajeSK.smsAvisoIIG()}😳 *NO PUEDO BANEAR AL OWNER @${ownerNumber} DE ${packname}*`}
const smsPropban4 = (number) => { return `${lenguajeGB.smsAvisoIIG()}*NO ES NECESARIO VOLVER A BANEAR A @${number} SI YA LO ESTÁ* 😊`}
const smsPropban5 = () => { return `${lenguajeSK['smsAvisoEG']()}*USUARIO BANEADO CON ÉXITO* 🙀`}
const smsPropban6 = (number, usr) => { return `${lenguajeSK.smsAvisoAG()}*@${number} ESTAS BANEADO/A POR @${usr} NO PUEDES USAR LOS COMANDOS HASTA QUE ALGUIEN REVIERTA EL BANEO* 😿`}
const smsPropban7 = (usedPrefix, command, number) => { return `${lenguajeSK['smsAvisoFG']()}*SURGIÓ UN ERROR, PUEDE SER QUE EL USUARIO NO ESTE EN MI BASE DE DATOS INTENTE ESCRIBIR ${usedPrefix + command} ${number}*\n\`\`\`SI EL ERROR CONTINÚA REPORTE ESTE COMANDO\`\`\``}

//owner-unbanuser.js
const smsPropdesban1 = (usedPrefix, command, bot) => { return `${lenguajeSK['smsAvisoMG']()}*ETIQUETE A ALGUIEN O RESPONDA AL MENSAJE DEL USUARIO O ESCRIBA EL NÚMERO QUE QUIERE DESBANEAR DE LOS COMANDOS*\n\n*EJEMPLO:*\n*${usedPrefix + command} @${bot}*`}
const smsPropdesban2 = (number) => { return `${lenguajeSK.smsAvisoIIG()}*NO ES NECESARIO VOLVER A DESBANEAR A @${number} SI YA LO ESTÁ* 😊`}
const smsPropdesban3 = () => { return `${lenguajeSK['smsAvisoEG']()}*USUARIO DESBANEADO CON ÉXITO* 💙`}
const smsPropdesban4 = (number, usr) => { return `${lenguajeSK.smsAvisoAG()}*@${number} ESTAS DESBANEADO/A POR @${usr} YA PUEDES USAR LOS COMANDOS!!* 🪄`}
const smsPropdesban5 = (usedPrefix, command, number) => { return `${lenguajeSK['smsAvisoFG']()}*SURGIÓ UN ERROR, PUEDE SER QUE EL USUARIO NO ESTE EN MI BASE DE DATOS INTENTE ESCRIBIR ${usedPrefix + command} ${number}*\n\`\`\`SI EL ERROR CONTINÚA REPORTE ESTE COMANDO\`\`\``}
...
//owner-unbanchat.js
const smsUnbanCH1 = () => { return lenguajeSK['smsAvisoFG']() + '🗂️ *ESTE CHAT NO ESTÁ REGISTRADO EN LA BASE DE DATOS*'}
const smsUnbanCH2 = () => { return lenguajeSK.smsAvisoAG() + '🌹 *ESTE CHAT NO ESTÁ BANEADO!!*'}
const smsUnbanCH3 = () => { return lenguajeSK['smsAvisoEG']() + '🪄 *ESTE CHAT FUE DESBANEADO, YA PUEDEN USAR LOS COMANDOS!!*'}
...
//propietario(a).js
const smsBlockUn1 = (usedPrefix, command, toUser) => { return lenguajeSK.smsMalused2() + `${usedPrefix + command} ${toUser}`}
const smsBlockUn2 = (comd, ownerNumber) => { return lenguajeSK.smsAvisoAG() + `*NO PUEDO ${comd} AL OWNER @${ownerNumber}*`}
const smsBlockUn3 = (comd, useB) => { return `*SE HA ${comd} A ${useB} CON ÉXITO*`}

//propietario(a).js
const smsRestarU1 = () => { return lenguajeSK['smsAvisoMG']() + '*ETIQUETE AL USUARIO, ESCRIBA SU NUMERO O RESPONDA AL MENSAJE PARA REINICIAR DATOS*'}
const smsRestarU2 = () => { return lenguajeSK['smsAvisoFG']() + '*EL NÚMERO QUE INGRESÓ NO ES VÁLIDO PARA REINICIAR LOS DATOS*'}
const smsRestarU3 = (number) => { return lenguajeSK['smsAvisoEG']() + `*SE REINICIÓ A @${number} DE LA BASE DE DATOS*`}

//propietario(a).js
const smsJoin1 = (usedPrefix, command) => { return lenguajeGB['smsAvisoMG']() + `*INGRESE EL ENLACE DE UN GRUPO*\n*EJEMPLO:*\n*${usedPrefix + command}* ${nna}`}
const smsJoin2 = () => { return lenguajeSK['smsAvisoEG']() + `${packname}\n*SE HA UNIDO AL GRUPO ✅*`}

//info.js
const smsReportGB1 = (usedPrefix, command) => { return lenguajeSK['smsAvisoMG']() + `*ESCRIBA UN REPORTE*\n*EJEMPLO:*\n\n*${usedPrefix + command}* _El comando ${usedPrefix}play no funciona._`}
const smsReportGB2 = () => { return lenguajeGB['smsAvisoFG']() + `✨ *MÍNIMO 10 CARACTERES PARA REALIZAR EL REPORTE*`}
const smsReportGB3 = () => { return lenguajeSK['smsAvisoFG']() + `😼 *MÁXIMO 1000 CARACTERES PARA REALIZAR EL REPORTE*`}
...
*⎔ Mensaje:*
*» ${text}*`.trim()}
const smsReportGB5 = () => { return lenguajeSK['smsAvisoEG']() + `*EL REPORTE HA SIDO ENVIADO A LOS DESARROLLADORES, DE SER NECESARIO TENDRÁ UNA RESPUESTA*`}

//buscadores.js
const smsGit1 = (usedPrefix, command) => { return `*INGRESE EL NOMBRE DE UN USUARIO DE GITHUB*\n*EJEMPLO*\n\n*${usedPrefix + command}* diegojadibot`}
